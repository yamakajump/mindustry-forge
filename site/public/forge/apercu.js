...
export async function drawPlan(panel, fetched = null) {
  ...
  if (!code.trim()) {
    fail(panel, "Ce schéma n'a pas de code enregistré.");
    return;
  }
  ...
  try {
    ...
    fail(panel, "Ce schéma ne se dessine pas : son code est illisible.");
    return;
  }

  if (!parsed?.tiles?.length) {
    fail(panel, "Ce schéma est vide.");
    return;
  }
...
async function fetchAndDraw(panel) {
  try {
    ...
    fail(panel, "Ce schéma n'a pas pu être chargé.");
  }
}
