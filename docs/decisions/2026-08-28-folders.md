...
The second of four related specs, and much the largest.

| Spec | What it adds |
|---|---|
| 1. Likes and favorites | A public like, a private favorite, the ordering that follows |
| **2. This one** | Folders: named, pictured, nested, and shareable by link |
| 3. Notes | A personal note on a schematic, and on why it is in a folder |
| 4. Liking a folder | The same gesture as spec 1, on folders, and a ranking of them |
...
Folders can hold folders, so this is a tree, not two fixed levels.
...
will. The name is validated through `IconController::draws()`, the same check the icon
route itself uses to decide whether to render or 404: a name that would not draw is
refused rather than stored and rendered as a broken image.
...
**The card is not built yet.** Sharing happens in Discord, and a link with no preview does
not get clicked. `Services/Cards/Card.php` already carries `SchematicCard` and `BlockCard`,
and a `FolderCard` beside them, carrying the name, the icon, the count of schematics and
four thumbnails, is the obvious next one to add, at `/d/{slug}/carte.jpg` by parity with
`/s/{slug}/carte.jpg`. Neither the class nor the route exists today.
...
Nothing here touches `BrowseController` or `browse.blade.php`, and nothing here touches
`analyse.js`, so **`EngineVersion` does not move and no stored analysis goes stale**.
Checked the same way: the checksum of `blocks.json` identical to the byte.
...
Implementation depends on spec 1, because a folder page reuses the tile and the like count
that spec 1 puts there, and on the `/schemas` rename that both build on.
