...
- `bilan.js` is not touched, so `EngineVersion` does not move and no stored analysis goes
  stale.
...
Six products, six icons, no duplicate possible, this document assumed: a schematic that
makes several things can win for several of them, and testing found it did, showing four
plans instead of six with two drawn twice. `HomeController::showcase()` now skips a
schematic already shown and lets its product fall through to the next one down, which is
where the guarantee this paragraph describes actually lives. Sandbox-fed and creative rows
are excluded the same way the browse page excludes them.
...
- **`bilan.js`, and therefore `EngineVersion`.** The checksum of `blocks.json` is compared
...
- `HomeTest` gains cases for the spread: six distinct products, no duplicate slug, no
  sandbox-fed row.
- `NavigationTest` covers the header, diffing `config/nav.php` against the `<nav>` markup
  in the Blade views, `index.html` and the two tool pages. It does not read the shelf: a
  tool added to `config/nav.php` and forgotten on the home page shelf is not caught by any
  test today.
...
`forge/bilan.js` is untouched, so `EngineVersion` does not move and no stored analysis
goes stale.
