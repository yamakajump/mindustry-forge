...
Both limits are enforced server side. The private note also shows a live count in its
field, because a form that refuses 1050 characters after the fact loses the fifty that
mattered; the folder caption is edited through a browser `prompt()`, which has no field to
put a count in, and enforces its 280 only server side.
...
- Somebody else's private note is invisible on the same schematic page: there is no route
  to read a note by itself, only to write one, so the page simply never queries another
  user's note when it renders.
