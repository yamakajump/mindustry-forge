...
def main() -> None:
    ...
    lines.append("")
    lines.append("## Ce qui est coché et reste incomplet")
    lines.append("")
    lines.append("Une case cochée dit que la classe est transcrite et mesurée, pas")
    lines.append("qu'aucun de ses blocs ne pose problème. Ce qui manque encore, nommé")
    lines.append("plutôt que laissé à découvrir :")
    lines.append("")
    lines.append("- **Un bloc ne retient qu'un liquide à la fois.** Le jeu, lui, garde un")
    lines.append("  compteur par liquide et n'utilise `current()` que pour décider ce")
    lines.append("  qu'il accepte. Ça ne se voit que sur les blocs qui en boivent deux :")
    lines.append("  `chemical-combustion-chamber` (ozone et arkycite), `pyrolysis-generator`")
    lines.append("  (scorie et arkycite), `neoplasia-reactor` (arkycite et eau). Les trois")
    lines.append("  sont d'Erekir, aucun n'est mesuré, et aucun ne tournera correctement")
    lines.append("  tant que le module liquide n'aura pas plusieurs cases. Les deux")
    lines.append("  foreuses à percussion sont dans le même cas pour leur **bonus** :")
    lines.append("  elles veulent de l'eau et de l'ozone en même temps, donc le bonus est")
    lines.append("  transcrit et non mesuré. Leur marche normale, elle, est mesurée.")
    lines.append("")
