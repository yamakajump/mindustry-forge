...
def main() -> None:
    ...
    lines.append("")
    lines.append("- **Les charges utiles n'existent pas.** Aucun bloc qui porte, pose ou")
    lines.append("  consomme une unité empaquetée n'est reproduit : `PayloadConveyor`,")
    lines.append("  `PayloadRouter`, `Reconstructor`, `UnitAssembler` et le reste de la")
    lines.append("  famille. Ils sont dans la liste, non cochés, et une schématique qui en")
    lines.append("  contient est lue comme si ces blocs ne faisaient rien.")
    lines.append("")
    lines.append("- **Les processeurs ne s'exécutent pas.** `LogicBlock` lit un programme")
    lines.append("  qui peut piloter n'importe quel bloc de la schématique. Rien de ça")
    lines.append("  n'est simulé, et ça ne le sera probablement jamais.")
    lines.append("")
