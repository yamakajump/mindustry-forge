/** Quelles faces de chaque transporteur donnent sur le vide, hors de la boite. */
...
for (let i = 0; i < g.nodes.length; i++) {
  ...
  // Les faces d entree : tout sauf celle de sortie.
  ...
  if (openInputs.length) {
    console.log(`  ${n.name.padEnd(16)} (${n.x},${n.y}) rot=${n.rotation} `
      + `faces libres hors boite: ${openInputs.map((s) => `${s.dx},${s.dy}`).join(" ")}`
      + `  entrees internes: ${g.into[i].length}`);
  }
}
