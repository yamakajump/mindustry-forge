...
`CHANGELOG.md` is regenerated by `release-please` on every release, so it should hold only
what the tool writes: nothing hand-added above the header survives the next merge. The
earliest entries in it are in French because the project's commit language changed to
English partway through; they are kept as written rather than rewritten, and this
paragraph is where that explanation lives instead.
