...
Adding one is [`docs/bench-scenarios.md`](docs/bench-scenarios.md): what to write, which
five files a scenario is made of, and what to do when you have no game jar and cannot
measure. Proving a new number is the thing this repository most needs a contributor to be
able to do.
...
The analyser itself is a static page and needs no server of its own beyond one that serves
plain files, for example `cd site/public && python -m http.server 8770`. Opening
`site/public/index.html` straight off the filesystem does not work: it imports ES modules,
and a browser refuses that import over `file://`.
...
release and appends the entry to `CHANGELOG.md`. It deploys nothing, and the three files
it writes are never edited by hand. [`docs/releasing.md`](docs/releasing.md) is the
runbook: what each step writes, how to check that production runs the commit the tag
names, and what to do when a release should not have been cut.

The earliest entries in `CHANGELOG.md` are in French because the project's commit language
changed to English partway through; they are kept as written rather than rewritten, and
